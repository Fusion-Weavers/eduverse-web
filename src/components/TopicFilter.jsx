...
import { IoSearchOutline, IoCloseCircleOutline, IoHeart, IoHeartOutline, IoFilterOutline, IoRefreshOutline } from 'react-icons/io5';
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
        ...
        <div className="lg:col-span-4 space-y-3">
          <label className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400 ml-1">
            <IoSearchOutline className="text-sm" /> {getUITranslation('search', currentLanguage)}
          </label>
        ...
        <div className="lg:col-span-5 space-y-3">
          <label className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-slate-400 ml-1">
            <IoFilterOutline className="text-sm" /> {getUITranslation('difficulty.title', currentLanguage) === 'difficulty.title' ? 'Difficulty' : getUITranslation('difficulty.title', currentLanguage)}
          </label>
          <div className="flex flex-wrap gap-2">
            {['all', 'beginner', 'intermediate', 'advanced'].map(level => (
              <button
                ...
                `}
              >
                {level === 'all' ? getUITranslation('all', currentLanguage) === 'all' ? 'Universal' : getUITranslation('all', currentLanguage) : getUITranslation(`difficulty.${level}`, currentLanguage)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-3 flex flex-wrap lg:flex-nowrap items-center gap-4">
          ...
          >
            ...
            <span>{getUITranslation('favorites', currentLanguage)}</span>
          </button>
